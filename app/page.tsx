...
export default function Home() {
  ...
  const [format, setFormat] = useState("Помогите выбрать"), [chaos, setChaos] = useState("Есть вопросы");
  const [name, setName] = useState(""), [contact, setContact] = useState(""), [consent, setConsent] = useState(false);
  ...
  function startBooking(selected = "Помогите выбрать") {
    ...
    setFormat(selected); setStatus("idle");setError("");setOpen(true);
  }
  useEffect(() => {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/enquiries", {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:requestId.current,name:name.trim(),contact:contact.trim(),chaos,format,consent,website:new FormData(event.currentTarget).get("website")})});
      const result = await response.json() as {error?: string; ok?: boolean};
      if(!response.ok || result.ok!==true) throw new Error(result.error || "Не получилось отправить заявку. Попробуй ещё раз.");
      setStatus("success");requestId.current="";setName("");setContact("");setConsent(false);
  ...
  return <>
    ...
    <main id="main">
      ...
      <section className="story-section section container" id="story">
        ...
        <div className="story-copy"><p className="large-copy">Я — филолог. И я за то, чтобы<br className="desktop-br"/> русский стал <mark>понятным.</mark></p><p>На моих занятиях можно задавать любые вопросы, ошибаться и пробовать снова. Вместо монолога — живой разговор, вместо бесконечной зубрёжки — поиск смысла.</p><p>Русский язык и литература для меня связаны: учимся замечать детали, понимать тексты и точно выражать свои мысли. И да, для юмора на уроке тоже найдётся место.</p><div className="story-tags"><span>Филология</span><span>Живой диалог</span><span>Авторские материалы</span></div></div>
      ...
      <section className="method-section section" id="method"><div className="container method-layout">
        <div className="chaos-poster"><span className="poster-label">У ВСЕГО ЕСТЬ СВОЁ МЕСТО</span><div className="chaos-letters" aria-label="Хаос">{["Х","А","О","С"].map(l=><span key={l}>{l}</span>)}</div><div className="chaos-arrow"><svg viewBox="0 0 100 120" aria-hidden="true"><path d="M34 5C8 57 102 48 61 72S12 47 43 56s30 34 9 49m-11-17 11 20 15-13"/></svg></div><div className="system-word">СИСТЕМА</div><p>Понятный маршрут.<br/>Твой темп. Всё по делу.</p><Doodle className="poster-star"/></div>
        <div className="shelves-wrap"><span className="eyebrow">03 / ОТ «НЕ ПОНИМАЮ» К «Я МОГУ»</span><h2>Разложим<br/><span className="underline-lime">по полочкам</span></h2><p className="section-description">У каждого шага своя задача. И на каждом я рядом.</p>
          <div className="shelves"><div className="shelf-row"><article className="shelf-item"><span className="step-number pink-number">01</span><div><h3>Знакомимся</h3><p>Твои цели и точка старта</p></div></article><article className="shelf-item"><span className="step-number lime-number">02</span><div><h3>Разбираемся</h3><p>Находим логику и связи</p></div></article></div>
            <div className="shelf-row"><article className="shelf-item practice-step"><span className="step-number white-number">03</span><div><h3>Пробуем на практике</h3><p>Закрепляем новое на понятных примерах</p></div><PencilLine size={34}/></article></div>
            <div className="shelf-row"><article className="shelf-item"><span className="step-number lime-number">04</span><div><h3>Разбираем ошибки</h3><p>Учимся замечать главное</p></div></article><article className="shelf-item"><span className="step-number pink-number">05</span><div><h3>Закрепляем</h3><p>Домашка и обратная связь</p></div></article></div>
          </div><span className="hand shelf-caption">и вот всё встало на свои места :)</span>
        </div>
      </div></section>
    ...
    <Dialog open={open} onOpenChange={value=>{if(status!=="sending")setOpen(value);}}><DialogContent className="booking-dialog" showCloseButton={false} onCloseAutoFocus={e=>{e.preventDefault();lastTrigger.current?.focus();}}>
      ...
      {status==="success" ? <div className="success-content"><span className="success-check"><Check size={35}/></span><DialogTitle>Первый шаг сделан!</DialogTitle><DialogDescription>Заявка сохранена. Твои контакты и пожелания переданы для связи по поводу занятий.</DialogDescription><p className="hand">С хаосом разберёмся вместе ♡</p><button className="button button-pink" onClick={()=>setOpen(false)}>Отлично <Check size={18}/></button></div> : <>
        ...
        <form onSubmit={submit} className="booking-form">
          ...
          <div className="form-field"><label htmlFor="student-contact">Где тебе написать?</label><input id="student-contact" name="contact" autoComplete="off" required minLength={5} maxLength={160} placeholder="@telegram, телефон или email" value={contact} onChange={e=>{setContact(e.target.value);requestId.current="";}}/><span className="field-hint">Например: @username или +7 999 123-45-67</span></div>
          ...
          <p className="chosen-format">Направление: <strong>{format}</strong></p>
          <label className="consent-label"><Checkbox checked={consent} onCheckedChange={v=>setConsent(v===true)} required aria-label="Согласие на обработку контактов"/><span>Согласен(-на) на обработку имени и контакта для обратной связи по этой заявке.</span></label>
